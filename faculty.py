...
from mid_marks import faculty_mid_marks_entry, faculty_mid_marks_analytics
...
def faculty_attendance_auto(df):
    ...
    attendance_statuses = [st.session_state.get(f'att_status_{idx}', None) for idx in class_students.index]
    marked_students = sum(1 for status in attendance_statuses if status is not None)
    ...
    if marked_students > 0:
        ...
        if marked_students < total_students:
            st.warning(f"⚠️ {total_students-marked_students} students are not updated yet. Please confirm before you submit.")
    else:
        st.warning("⚠️ No attendance marked yet. Mark student statuses and retry submission.")
...
def faculty_dashboard():
    ...
    with row3_c1:
        if st.button("✏️ Mid Marks Entry"):
            st.session_state.faculty_tab = "mid_marks_entry"
    with row3_c2:
    ...
    # ================= MID MARKS ENTRY =================
    elif st.session_state.faculty_tab == "mid_marks_entry":
        # Load student data from grades database
        if os.path.exists(GRADES_FILE):
            df = pd.read_csv(GRADES_FILE)
            df.columns = df.columns.str.strip().str.lower()
    ...
    # ================= MID MARKS ANALYTICS =================
    elif st.session_state.faculty_tab == "mid_marks_analytics":
        # Load student data from grades database
        if os.path.exists(GRADES_FILE):
            df = pd.read_csv(GRADES_FILE)
            df.columns = df.columns.str.strip().str.lower()
            ...
            required_cols = ['rollno', 'name', 'year', 'semester']
            if all(col in df.columns for col in required_cols):
                faculty_mid_marks_analytics(df)
            else:
                st.error("❌ Grades database is missing required columns")
        else:
            st.warning("⚠️ Grades database not found. Please upload student data first.")
